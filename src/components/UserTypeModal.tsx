      ...
      <div className="bg-white rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8 w-full max-w-xs sm:max-w-sm lg:max-w-md flex flex-col items-center">
        ...
        <div className="flex flex-col gap-2 sm:gap-3 lg:gap-4 w-full mb-3 sm:mb-4 lg:mb-6">
          {userTypeOptions.map(option => (
            <button
              key={option.value}
              className={`flex items-center w-full py-2.5 sm:py-3 lg:py-3 px-3 sm:px-4 rounded-xl border-2 transition-all text-left text-sm sm:text-base lg:text-lg font-medium focus:outline-none focus:ring-2 focus:ring-gray-400
                ${selected === option.value ? 'border-black bg-gray-50 shadow' : 'border-gray-200 bg-gray-50 hover:bg-gray-100'}`}
        ...
        <button
          className={`w-full py-2 sm:py-2.5 lg:py-2.5 rounded-lg font-semibold text-white transition-colors text-sm sm:text-base lg:text-lg shadow-md
            ${selected ? 'bg-black hover:bg-gray-800' : 'bg-gray-300 cursor-not-allowed'}`}
