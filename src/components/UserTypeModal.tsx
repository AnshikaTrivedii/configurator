...
const UserTypeModal: React.FC<UserTypeModalProps> = ({ isOpen, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 animate-fade-in p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-sm sm:max-w-md flex flex-col items-center">
        <h2 className="text-xl sm:text-2xl font-bold mb-2 text-gray-900 text-center">Select User Type</h2>
        <p className="text-gray-500 mb-4 sm:mb-6 text-center max-w-xs text-sm sm:text-base">Choose your user type to see the most relevant pricing for your needs. You can only select this once per session.</p>
        <div className="flex flex-col gap-3 sm:gap-4 w-full mb-4 sm:mb-6">
          {userTypeOptions.map(option => (
            <button
              key={option.value}
              className={`flex items-center w-full py-3 px-4 rounded-xl border-2 transition-all text-left text-base sm:text-lg font-medium focus:outline-none focus:ring-2 focus:ring-blue-400
        ...
        <button
          className={`w-full py-2.5 rounded-lg font-semibold text-white transition-colors text-base sm:text-lg shadow-md
