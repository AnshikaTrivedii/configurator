import { Product } from '../types';

// Temporary series-level feature flag: set to true when Jumbo needs to be re-enabled.
export const JUMBO_SERIES_ACTIVE = false;

export const products: Product[] = [

  {
    id: "orion-p153-flexible-indoor",
    name: "Nebula Series Indoor P1.53",
    category: "Flexible Series",
    enabled: true,
    image: "/products/flexible-series/Flexible Series Indoor P1.5.jpeg",
    resolution: {
      width: 208,
      height: 104
    },
    price: 24412,
    siChannelPrice: 22133,
    resellerPrice: 20750,
    prices: {
      cabinet: { endCustomer: 24412, siChannel: 22133, reseller: 20750 },
      curveLock: { endCustomer: 0, siChannel: 0, reseller: 0 } // Not applicable for flexible series
    },

    cabinetDimensions: {
      width: 320,
      height: 160
    },
    moduleDimensions: {
      width: 320,
      height: 160
    },
    moduleResolution: {
      width: 208,
      height: 104
    },
    moduleQuantity: 1, // Single flexible module
    pixelPitch: 1.53,
    pixelDensity: 427185,
    brightness: 800,
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 600,
    avgPowerConsumption: 200,
    // PDF lists 32 kg per sqm; stored here so UI can show a comparable weight figure.
    weightPerCabinet: 32,
    ledType: "Black SMD1212",
    driveMethod: "1/43",
    viewingAngle: "H 140°, V 140°",
    voltage: "AC 110/220V",
    serviceAccess: "Total Front Access",
    ipGrade: "IP31",
    operatingTemperature: "0°C~50°C",
    operatingHumidity: "10~90%RH",
    storageTempHumidity: "0°C~60°C, 10~90%RH",
    lifeTime: 100000,
    cabinetMaterial: "Sheet metal",
    pdf: "https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Nebula+Series/Nebula+Series-+P1.53mm.pdf"
  },

  {
    id: "orion-p186-flexible-indoor",
    name: "Nebula Series Indoor P1.86",
    category: "Flexible Series",
    enabled: true,
    image: "/products/flexible-series/Flexible Series Indoor P1.8.jpeg",
    resolution: {
      width: 172,
      height: 86
    },
    price: 15467,
    siChannelPrice: 15467,
    resellerPrice: 14500,
    prices: {
      cabinet: { endCustomer: 15467, siChannel: 15467, reseller: 14500 },
      curveLock: { endCustomer: 0, siChannel: 0, reseller: 0 } // Not applicable for flexible series
    },

    cabinetDimensions: {
      width: 320,
      height: 160
    },
    moduleDimensions: {
      width: 320,
      height: 160
    },
    moduleResolution: {
      width: 172,
      height: 86
    },
    moduleQuantity: 1, // Single flexible module
    pixelPitch: 1.86,
    pixelDensity: 288906,
    brightness: 800,
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 600,
    avgPowerConsumption: 200,
    // PDF lists 32 kg per sqm; stored here so UI can show a comparable weight figure.
    weightPerCabinet: 32,
    ledType: "Black SMD1515",
    driveMethod: "1/43",
    viewingAngle: "H 140°, V 140°",
    voltage: "AC 110/220V",
    serviceAccess: "Total Front Access",
    ipGrade: "IP31",
    operatingTemperature: "0°C~50°C",
    operatingHumidity: "10~90%RH",
    storageTempHumidity: "0°C~60°C, 10~90%RH",
    lifeTime: 100000,
    cabinetMaterial: "Sheet metal",
    pdf: "https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Nebula+Series/Nebula+Series-+P1.86mm.pdf"
  },

  {
    id: "orion-p25-flexible-indoor",
    name: "Nebula Series Indoor P2.5",
    category: "Flexible Series",
    enabled: true,
    image: "/products/flexible-series/Flexible Series Indoor P2.5.jpeg",
    resolution: {
      width: 128,
      height: 64
    },
    price: 11000,
    siChannelPrice: 11733,
    resellerPrice: 11000,
    prices: {
      cabinet: { endCustomer: 11000, siChannel: 11733, reseller: 11000 },
      curveLock: { endCustomer: 0, siChannel: 0, reseller: 0 } // Not applicable for flexible series
    },

    cabinetDimensions: {
      width: 320,
      height: 160
    },
    moduleDimensions: {
      width: 320,
      height: 160
    },
    moduleResolution: {
      width: 128,
      height: 64
    },
    moduleQuantity: 1, // Single flexible module
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 800,
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 600,
    avgPowerConsumption: 200,
    // PDF lists 32 kg per sqm; stored here so UI can show a comparable weight figure.
    weightPerCabinet: 32,
    ledType: "Black SMD2020",
    driveMethod: "1/32",
    viewingAngle: "H 140°, V 140°",
    voltage: "AC 110/220V",
    serviceAccess: "Total Front Access",
    ipGrade: "IP31",
    operatingTemperature: "0°C~50°C",
    operatingHumidity: "10~90%RH",
    storageTempHumidity: "0°C~60°C, 10~90%RH",
    lifeTime: 100000,
    cabinetMaterial: "Sheet metal",
    pdf: "https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Nebula+Series/Nebula+Series-+P2.5mm.pdf"
  },

  {
    id: "orion-p3-flexible-indoor",
    name: "Nebula Series Indoor P3",
    category: "Flexible Series",
    enabled: true,
    image: "/products/flexible-series/Flexible Series Indoor P3.jpeg",
    resolution: {
      width: 80,
      height: 40
    },
    price: 8100,
    siChannelPrice: 10800,
    resellerPrice: 10125,
    prices: {
      cabinet: { endCustomer: 8100, siChannel: 10800, reseller: 10125 },
      curveLock: { endCustomer: 0, siChannel: 0, reseller: 0 } // Not applicable for flexible series
    },

    cabinetDimensions: {
      width: 240,
      height: 120
    },
    moduleDimensions: {
      width: 240,
      height: 120
    },
    moduleResolution: {
      width: 80,
      height: 40
    },
    moduleQuantity: 1, // Single flexible module
    pixelPitch: 3.0,
    pixelDensity: 111111,
    brightness: 800,
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 600,
    avgPowerConsumption: 200,
    // PDF lists 32 kg per sqm; stored here so UI can show a comparable weight figure.
    weightPerCabinet: 32,
    ledType: "Black SMD2121",
    driveMethod: "1/32",
    viewingAngle: "H 140°, V 140°",
    voltage: "AC 110/220V",
    serviceAccess: "Total Front Access",
    ipGrade: "IP31",
    operatingTemperature: "0°C~50°C",
    operatingHumidity: "10~90%RH",
    storageTempHumidity: "0°C~60°C, 10~90%RH",
    lifeTime: 100000,
    cabinetMaterial: "Sheet metal",
    pdf: "https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Nebula+Series/Nebula+Series-+P3mm.pdf"
  },

  {
    id: "nebula-cabinet-p2.9",
    name: "Nebula Series Cabinet Base P2.9",
    category: "Flexible Series",
    enabled: true,
    image: "/products/flexible-series/1 (1).png", // Using existing placeholder, update if new image provided
    resolution: {
      width: 168,
      height: 168
    },
    price: 21100,
    siChannelPrice: 19300,
    resellerPrice: 18300,
    prices: {
      cabinet: { endCustomer: 21100, siChannel: 19300, reseller: 18300 },
      curveLock: { endCustomer: 0, siChannel: 0, reseller: 0 } // Not applicable for flexible series
    },
    cabinetDimensions: {
      width: 500,
      height: 500
    },
    moduleDimensions: {
      width: 250,
      height: 250
    },
    moduleResolution: {
      width: 84,
      height: 84
    },
    moduleQuantity: 4, // 2x2
    pixelPitch: 2.97,
    pixelDensity: 112896,
    brightness: 800,
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 800,
    avgPowerConsumption: 280,
    weightPerCabinet: 8,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Nebula+Series/Nebula+Series-+P2.9mm.pdf',
    ledType: "Black SMD 2121 / Black SMD 2020",
    driveMethod: "1/21",
    viewingAngle: "H 140°, V 140°",
    serviceAccess: "FRONT",
    ipGrade: "IP31/43",
    operatingTemperature: "0°C~50°C",
    operatingHumidity: "10~90%RH",
    storageTempHumidity: "0°C~60°C, 10~90%RH",
    cabinetMaterial: "Die-casting Aluminum",
    controller: "Novastar",
    contrastRatio: "10000:1",
    colorTemperature: "3200-9300 Optional",
    frameChangingFrequently: "50/60",
    certificates: "CE/BIS/CCC/FCC/CB"
  },

  {
    id: 'bellatrix-indoor-cob-p0.9',
    name: 'Sigma Core P0.9',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor COB P0.9.png',
    resolution: { width: 640, height: 360 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 160, height: 180 },
    moduleQuantity: 8, // 4x2
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 0.9375,
    pixelDensity: 1137778,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/60',
    connectionTechnology: 'Common Anode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Sigma+Core-+P0.9mm.pdf',
    price: 49300,
    siChannelPrice: 44370,
    resellerPrice: 41905,
  },
  {
    id: 'bellatrix-indoor-cob-p1.25',
    name: 'Sigma Core P1.25',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor COB P1.25.png',
    resolution: { width: 480, height: 270 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 120, height: 135 },
    moduleQuantity: 8, // 4x2
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/60',
    connectionTechnology: 'Common Anode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Sigma+Core-+P1.25mm.pdf',
    price: 27200,
    siChannelPrice: 24480,
    resellerPrice: 23120,
  },
  {
    id: 'bellatrix-indoor-cob-p1.5',
    name: 'Sigma Core P1.5',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor COB P1.5.png',
    resolution: { width: 384, height: 216 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 96, height: 108 },
    moduleQuantity: 8, // 4x2
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 1.5625,
    pixelDensity: 409600,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/48',
    connectionTechnology: 'Common Anode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Sigma+Core-+P1.56mm.pdf',
    price: 24300,
    siChannelPrice: 21870,
    resellerPrice: 20655,
  },

  {
    id: 'bellatrix-indoor-smd-p1.25',
    name: 'Astra Core-I P1.25',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor SMD P1.25.png',
    resolution: { width: 512, height: 384 },
    cabinetDimensions: { width: 640, height: 480 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 256, height: 128 },
    moduleQuantity: 6, // 2x3
    moduleGrid: { columns: 2, rows: 3 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    brightness: 800,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Chuangliang/rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-I+P1.25mm.pdf',
    price: 21300,
    siChannelPrice: 19170,
    resellerPrice: 18105,
    cabinetVariations: [
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 512, height: 384 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 512, height: 512 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-indoor-smd-p1.5',
    name: 'Astra Core-I P1.5',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor SMD P1.5.png',
    resolution: { width: 426, height: 320 },
    cabinetDimensions: { width: 640, height: 480 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 213, height: 107 },
    moduleQuantity: 6, // 2x3
    moduleGrid: { columns: 2, rows: 3 },
    pixelPitch: 1.5,
    pixelDensity: 444000,
    brightness: 800,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Chuangliang/rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-I+P1.5mm.pdf',
    price: 16900,
    siChannelPrice: 15210,
    resellerPrice: 14365,
    cabinetVariations: [
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 426, height: 320 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 426, height: 426 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-indoor-smd-p1.8',
    name: 'Astra Core-I P1.8',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor SMD P1.8.png',
    resolution: { width: 344, height: 258 },
    cabinetDimensions: { width: 640, height: 480 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 172, height: 86 },
    moduleQuantity: 6, // 2x3
    moduleGrid: { columns: 2, rows: 3 },
    pixelPitch: 1.86,
    pixelDensity: 284444,
    brightness: 800,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Chuangliang/rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-I+P1.86mm.pdf',
    price: 16200,
    siChannelPrice: 14580,
    resellerPrice: 13770,
    cabinetVariations: [
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 344, height: 258 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 344, height: 344 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-indoor-smd-p2.5',
    name: 'Astra Core-I P2.5',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Indoor SMD P2.5.png',
    resolution: { width: 256, height: 192 },
    cabinetDimensions: { width: 640, height: 480 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 6, // 2x3
    moduleGrid: { columns: 2, rows: 3 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 900,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 350,
    weightPerCabinet: 8,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-I+P2.5mm.pdf',
    price: 8800,
    siChannelPrice: 7920,
    resellerPrice: 7480,
    cabinetVariations: [
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 256, height: 192 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 256, height: 256 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-outdoor-smd-p2.5',
    name: 'Astra Core-O P2.5',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Outdoor SMD P2.5.png',
    resolution: { width: 384, height: 384 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 4000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 350,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-O+P2.5mm.pdf',
    price: 13200,
    siChannelPrice: 11880,
    resellerPrice: 11220,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 384, height: 384 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 256, height: 256 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-outdoor-smd-p3',
    name: 'Astra Core-O P3',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Outdoor SMD P3.png',
    resolution: { width: 312, height: 312 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 104, height: 52 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 3.0,
    pixelDensity: 111111,
    brightness: 4500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 450,
    avgPowerConsumption: 250,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-O+P3mm.pdf',
    price: 10600,
    siChannelPrice: 9540,
    resellerPrice: 9010,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 312, height: 312 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 208, height: 208 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-outdoor-smd-p4',
    name: 'Astra Core-O P4',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Outdoor SMD P4.png',
    resolution: { width: 240, height: 240 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 4.0,
    pixelDensity: 62500,
    brightness: 5000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 400,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-O+P4mm.pdf',
    price: 8100,
    siChannelPrice: 7290,
    resellerPrice: 6885,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 240, height: 240 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 160, height: 160 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-outdoor-smd-p6.6',
    name: 'Astra Core-O P6.6',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Outdoor SMD P6.6.png',
    resolution: { width: 144, height: 144 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 48, height: 24 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 6.6,
    pixelDensity: 23104,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 400,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-O+P6.6mm.pdf',
    price: 7100,
    siChannelPrice: 6390,
    resellerPrice: 6035,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 144, height: 144 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 96, height: 96 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },
  {
    id: 'bellatrix-outdoor-smd-p10',
    name: 'Astra Core-O P10',
    category: 'Core Series',
    enabled: true,
    image: '/products/bellatrix-series/Bellatrix Series Outdoor SMD P10.png',
    resolution: { width: 96, height: 96 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 32, height: 16 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 10.0,
    pixelDensity: 10000,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 750,
    avgPowerConsumption: 350,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 50000,
    mtbf: 5000,
    ledChip: 'Atenti SMD',
    pcb: '2 layer',
    driverIC: 'ICN/SM/DP',
    powerSupply: 'Rong',
    controller: 'Nova',
    cabinetMaterial: 'Die-Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Core+series/Astra+Core-O+P10mm.pdf',
    price: 5900,
    siChannelPrice: 5310,
    resellerPrice: 5015,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 96, height: 96 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 64, height: 64 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'orion-p18-digital-standee',
    name: 'Omega-I Lite P1.8',
    category: 'Digital Standee Series',
    enabled: true,
    digitalStandeeCabinetGrid: { columns: 2, rows: 11 },
    image: '/products/standee-series/Indoor Lite P1.8.png',
    resolution: { width: 344, height: 946 },
    cabinetDimensions: { width: 665, height: 1785 }, // 665mm x 1785mm
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 176, height: 176 },
    moduleQuantity: 12,
    pixelPitch: 1.8,
    pixelDensity: 284444,
    brightness: 780, // cd/m²
    refreshRate: 3840, // Hz
    environment: 'indoor',
    maxPowerConsumption: 400, // <400W
    avgPowerConsumption: 120, // <120W
    weightPerCabinet: 45, // kg
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Omega+Series/Omega-I+Lite+-+P1.8mm.pdf',
    ledType: 'SMD',
    driverIC: 'ICN/MBI/SM',
    viewingAngle: 'H140°, V140°',
    colorDepth: '14-16 bit',
    frameChangingFrequently: '>60 frames/sec',
    voltage: '110V-220V',
    serviceAccess: 'REAR',
    ipGrade: 'IP31/43',
    operatingTemperature: '-0°C~40°C 10-90%RH',
    storageTempHumidity: '-0°C~40°C 10-90%RH',
    lifeTime: 100000,
    controller: 'Novastar TB40',
    cabinetMaterial: 'Aluminum',
    screenResolution: '344×946',
    activeDisplayArea: '640mm x 1760mm',
    dimensionConstraints: {
      series: 'Digital Standee Series',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 640,
      minHeight: 1760,
      maxWidth: 1280,
      maxHeight: 1920,
      validationMessage: 'For Digital Standee Series, screen size must be between 640x1760mm and 1280x1920mm and in multiples of 320x160mm module size.',
    },
    price: 183800,
    siChannelPrice: 166700,
    resellerPrice: 156300,
  },
  {
    id: 'orion-p18-digital-standee-2',
    name: 'Omega-I Plus P1.8',
    category: 'Digital Standee Series',
    enabled: true,
    digitalStandeeCabinetGrid: { columns: 3, rows: 11 },
    image: '/products/standee-series/Indoor Plus P1.8.png',
    resolution: { width: 516, height: 946 },
    cabinetDimensions: { width: 985, height: 1785 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 160, height: 80 },
    moduleQuantity: 12,
    pixelPitch: 1.8,
    pixelDensity: 288578,
    brightness: 780,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 400,
    avgPowerConsumption: 120,
    weightPerCabinet: 65,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Omega+Series/Omega-I+Plus-+P1.8mm.pdf',
    ledType: 'SMD',
    driverIC: 'ICN/MBI/SM',
    viewingAngle: 'H140°, V140°',
    colorDepth: '14-16 bit',
    frameChangingFrequently: '>60 frames/sec',
    voltage: '110V-220V',
    serviceAccess: 'REAR',
    ipGrade: 'IP31/43',
    operatingTemperature: '-0°C~40°C 10-90%RH',
    storageTempHumidity: '-0°C~40°C 10-90%RH',
    lifeTime: 100000,
    controller: 'Novastar TB40',
    cabinetMaterial: 'Aluminum',
    screenResolution: '344 dots x 946 dots',
    activeDisplayArea: '640mm x 1760mm',
    dimensionConstraints: {
      series: 'Digital Standee Series',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 640,
      minHeight: 1760,
      maxWidth: 1280,
      maxHeight: 1920,
      validationMessage: 'For Digital Standee Series, screen size must be between 640x1760mm and 1280x1920mm and in multiples of 320x160mm module size.',
    },
    price: 242600,
    siChannelPrice: 220000,
    resellerPrice: 206300,
  },
  {
    id: 'orion-p25-digital-standee',
    name: 'Omega-I Lite P2.5',
    category: 'Digital Standee Series',
    enabled: true,
    digitalStandeeCabinetGrid: { columns: 2, rows: 11 },
    image: '/products/standee-series/Indoor Lite P2.5.png',
    resolution: { width: 256, height: 704 },
    cabinetDimensions: { width: 665, height: 1785 }, // 665mm x 1785mm
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 12,
    pixelPitch: 2.5,
    pixelDensity: 275625,
    brightness: 780, // cd/m²
    refreshRate: 3840, // Hz
    environment: 'indoor',
    maxPowerConsumption: 400, // <400W
    avgPowerConsumption: 120, // <120W
    weightPerCabinet: 45, // kg
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Omega+Series/Omega-I+Lite+-+P2.5mm.pdf',
    ledType: 'SMD',
    driverIC: 'ICN/MBI/SM',
    viewingAngle: 'H140°, V140°',
    colorDepth: '14-16 bit',
    frameChangingFrequently: '>60 frames/sec',
    voltage: '110V-220V',
    serviceAccess: 'REAR',
    ipGrade: 'IP31/43',
    operatingTemperature: '-0°C~40°C 10-90%RH',
    storageTempHumidity: '-0°C~40°C 10-90%RH',
    lifeTime: 100000,
    controller: 'Novastar TB40',
    cabinetMaterial: 'Aluminum',
    screenResolution: '256×704',
    activeDisplayArea: '640mm x 1760mm',
    dimensionConstraints: {
      series: 'Digital Standee Series',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 640,
      minHeight: 1760,
      maxWidth: 1280,
      maxHeight: 1920,
      validationMessage: 'For Digital Standee Series, screen size must be between 640x1760mm and 1280x1920mm and in multiples of 320x160mm module size.',
    },
    price: 139700,
    siChannelPrice: 126700,
    resellerPrice: 118800,
  },
  {
    id: 'orion-p25-digital-standee-2',
    name: 'Omega-I Plus P2.5',
    category: 'Digital Standee Series',
    enabled: true,
    digitalStandeeCabinetGrid: { columns: 3, rows: 11 },
    image: '/products/standee-series/Indoor Plus P2.5.png',
    resolution: { width: 384, height: 704 },
    cabinetDimensions: { width: 985, height: 1785 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 12,
    pixelPitch: 2.5,
    pixelDensity: 159744,
    brightness: 780,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 400,
    avgPowerConsumption: 120,
    weightPerCabinet: 65,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Omega+Series/Omega-I+Plus+-+P2.5mm.pdf',
    ledType: 'SMD',
    driverIC: 'ICN/MBI/SM',
    viewingAngle: 'H140°, V140°',
    colorDepth: '14-16 bit',
    frameChangingFrequently: '>60 frames/sec',
    voltage: '110V-220V',
    serviceAccess: 'REAR',
    ipGrade: 'IP31/43',
    operatingTemperature: '-0°C~40°C 10-90%RH',
    storageTempHumidity: '-0°C~40°C 10-90%RH',
    lifeTime: 100000,
    controller: 'Novastar TB40',
    cabinetMaterial: 'Aluminum',
    screenResolution: '256 dots x 704 dots',
    activeDisplayArea: '640mm x 1760mm',
    dimensionConstraints: {
      series: 'Digital Standee Series',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 640,
      minHeight: 1760,
      maxWidth: 1280,
      maxHeight: 1920,
      validationMessage: 'For Digital Standee Series, screen size must be between 640x1760mm and 1280x1920mm and in multiples of 320x160mm module size.',
    },
    price: 183800,
    siChannelPrice: 166700,
    resellerPrice: 156300,
  },
  {
    id: 'orion-p4-digital-standee',
    name: 'Omega-O Lite P4',
    category: 'Digital Standee Series',
    enabled: true,
    digitalStandeeCabinetGrid: { columns: 2, rows: 11 },
    image: '/products/standee-series/Outdoor Lite P4.png',
    resolution: { width: 160, height: 440 },
    cabinetDimensions: { width: 665, height: 1785 }, // 665mm x 1785mm
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 12,
    pixelPitch: 4.0,
    pixelDensity: 160000,
    brightness: 4500, // cd/m²
    refreshRate: 3840, // Hz
    environment: 'outdoor',
    maxPowerConsumption: 750, // <750W
    avgPowerConsumption: 400, // <400W
    weightPerCabinet: 60, // kg
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Omega+Series/Omega-O+Lite+-+P4mm.pdf',
    ledType: 'SMD',
    driverIC: 'ICN/MBI/SM',
    viewingAngle: 'H140°, V140°',
    colorDepth: '14-16 bit',
    frameChangingFrequently: '>60 frames/sec',
    voltage: '110V-220V',
    serviceAccess: 'REAR',
    ipGrade: 'IP65/43',
    operatingTemperature: '-0°C~40°C 10-90%RH',
    storageTempHumidity: '-0°C~40°C 10-90%RH',
    lifeTime: 100000,
    controller: 'Novastar TB40',
    cabinetMaterial: 'Aluminum',
    screenResolution: '160×440',
    activeDisplayArea: '640mm x 1760mm',
    dimensionConstraints: {
      series: 'Digital Standee Series',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 640,
      minHeight: 1760,
      maxWidth: 1280,
      maxHeight: 1920,
      validationMessage: 'For Digital Standee Series, screen size must be between 640x1760mm and 1280x1920mm and in multiples of 320x160mm module size.',
    },
    price: 161800,
    siChannelPrice: 146700,
    resellerPrice: 137500,
  },
  {
    id: 'orion-p4-digital-standee-2',
    name: 'Omega-O Plus P4',
    category: 'Digital Standee Series',
    enabled: true,
    digitalStandeeCabinetGrid: { columns: 3, rows: 11 },
    image: '/products/standee-series/Outdoor Plus P4mm.png',
    resolution: { width: 240, height: 440 },
    cabinetDimensions: { width: 985, height: 1785 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 12,
    pixelPitch: 4.0,
    pixelDensity: 62400,
    brightness: 4500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 750,
    avgPowerConsumption: 400,
    weightPerCabinet: 90,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Omega+Series/Omega-O+Plus+-+P4mm.pdf',
    ledType: 'SMD',
    driverIC: 'ICN/MBI/SM',
    viewingAngle: 'H140°, V140°',
    colorDepth: '14-16 bit',
    frameChangingFrequently: '>60 frames/sec',
    voltage: '110V-220V',
    serviceAccess: 'REAR',
    ipGrade: 'IP65/43',
    operatingTemperature: '-0°C~40°C 10-90%RH',
    storageTempHumidity: '-0°C~40°C 10-90%RH',
    lifeTime: 100000,
    controller: 'Novastar TB40',
    cabinetMaterial: 'Aluminum',
    screenResolution: '160 dots x 440 dots',
    activeDisplayArea: '640mm x 1760mm',
    dimensionConstraints: {
      series: 'Digital Standee Series',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 640,
      minHeight: 1760,
      maxWidth: 1280,
      maxHeight: 1920,
      validationMessage: 'For Digital Standee Series, screen size must be between 640x1760mm and 1280x1920mm and in multiples of 320x160mm module size.',
    },
    price: 198500,
    siChannelPrice: 180000,
    resellerPrice: 168800,
  },

  {
    id: 'rigel-p2.5-outdoor',
    name: 'Astra Edge-O P2.5',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Outdoor P2.5.png',
    resolution: { width: 384, height: 384 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 18, // 3x6 modules
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 5000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-O+P2.5mm.pdf',
    price: 15400,
    siChannelPrice: 13860,
    resellerPrice: 13090,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 384, height: 384 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 256, height: 256 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-p3-outdoor',
    name: 'Astra Edge-O P3',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Outdoor P3.png',
    resolution: { width: 312, height: 312 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 104, height: 52 },
    moduleQuantity: 18, // 3x6 modules
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 3.0,
    pixelDensity: 160000,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 450,
    avgPowerConsumption: 200,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-O+P3mm.pdf',
    price: 13200,
    siChannelPrice: 11880,
    resellerPrice: 11220,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 312, height: 312 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 208, height: 208 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-p4-outdoor',
    name: 'Astra Edge-O P4',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Outdoor P4.png',
    resolution: { width: 240, height: 240 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 18, // 3x6 modules
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 4.0,
    pixelDensity: 62500,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-O+P4mm.pdf',
    price: 10300,
    siChannelPrice: 9270,
    resellerPrice: 8755,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 240, height: 240 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 160, height: 160 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-p6.6-outdoor',
    name: 'Astra Edge-O P6.6',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Outdoor P6.6.png',
    resolution: { width: 144, height: 144 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 48, height: 24 },
    moduleQuantity: 18, // 3x6 modules
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 6.6,
    pixelDensity: 23104,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-O+P6.6mm.pdf',
    price: 8300,
    siChannelPrice: 7470,
    resellerPrice: 7055,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 144, height: 144 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 96, height: 96 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-p10-outdoor',
    name: 'Astra Edge-O P10',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Outdoor P10.png',
    resolution: { width: 96, height: 96 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 32, height: 16 },
    moduleQuantity: 18, // 3x6 modules
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 10.0,
    pixelDensity: 10000,
    brightness: 6000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 27,
    viewingAngle: '140/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-O+P10mm.pdf',
    price: 6600,
    siChannelPrice: 5940,
    resellerPrice: 5610,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 96, height: 96 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 64, height: 64 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'modular-outdoor-p3.91',
    name: 'Rigel Plus Outdoor P3.91',
    category: 'Modular Series',
    rigelVariant: 'plus',
    enabled: true,
    image: '/products/modular-series/P3.91.png',
    resolution: { width: 256, height: 256 },
    cabinetDimensions: { width: 1000, height: 1000 },
    moduleDimensions: { width: 500, height: 250 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 8,
    pixelPitch: 3.91,
    pixelDensity: 65536,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-plus/Rigel+Plus+P3.91mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD1921 Copper Wire / Gold Wire',
    viewingAngle: 'H:140°, V:140°',
    colorDepth: '16 bit',
    voltage: 'AC110V/220V ±10%',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    operatingHumidity: '10%~95%RH',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '5000:1',
    driveMethod: '1:8 Scan',
    lifeTime: 100000,
    certificates: 'CE, BIS, ROHS',
    price: 12000,
    siChannelPrice: 10800,
    resellerPrice: 10200,
  },

  {
    id: 'modular-outdoor-p4.81',
    name: 'Rigel Plus Outdoor P4.81',
    category: 'Modular Series',
    rigelVariant: 'plus',
    enabled: true,
    image: '/products/modular-series/P4.81.png',
    resolution: { width: 208, height: 208 },
    cabinetDimensions: { width: 1000, height: 1000 },
    moduleDimensions: { width: 500, height: 250 },
    moduleResolution: { width: 104, height: 52 },
    moduleQuantity: 8,
    pixelPitch: 4.81,
    pixelDensity: 43264,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-plus/Rigel+Plus+P4.81mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar Copper Wire / Gold Wire SMD1921',
    viewingAngle: 'H:140°, V:140°',
    colorDepth: '16 bit',
    voltage: 'AC110V/220V ±10%',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    operatingHumidity: '10%~95%RH',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '6000:1',
    driveMethod: '1:7 Scan',
    lifeTime: 100000,
    certificates: 'CE, BIS, ROHS',
    price: 11000,
    siChannelPrice: 9900,
    resellerPrice: 9350,
  },

  {
    id: 'modular-outdoor-p6.25',
    name: 'Rigel Plus Outdoor P6.25',
    category: 'Modular Series',
    rigelVariant: 'plus',
    enabled: true,
    image: '/products/modular-series/P6.25.png',
    resolution: { width: 160, height: 160 },
    cabinetDimensions: { width: 1000, height: 1000 },
    moduleDimensions: { width: 500, height: 250 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 8,
    pixelPitch: 6.25,
    pixelDensity: 25600,
    brightness: 6000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-plus/Rigel+Plus+P6.25mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD2727 Copper Wire / Gold Wire',
    viewingAngle: 'H:140°, V:140°',
    colorDepth: '16 bit',
    voltage: 'AC110V/220V ±10%',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    operatingHumidity: '10%~95%RH',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '5000:1',
    driveMethod: '1:8 Scan',
    lifeTime: 100000,
    certificates: 'CE, BIS, ROHS',
    price: 10200,
    siChannelPrice: 9180,
    resellerPrice: 8670,
  },

  {
    id: 'modular-outdoor-p7.81',
    name: 'Rigel Plus Outdoor P7.81',
    category: 'Modular Series',
    rigelVariant: 'plus',
    enabled: true,
    image: '/products/modular-series/P7.81.png',
    resolution: { width: 128, height: 128 },
    cabinetDimensions: { width: 1000, height: 1000 },
    moduleDimensions: { width: 500, height: 250 },
    moduleResolution: { width: 64, height: 32 },
    moduleQuantity: 8,
    pixelPitch: 7.81,
    pixelDensity: 16384,
    brightness: 6000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-plus/Rigel+Plus+P7.81mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD2727 Copper Wire / Gold Wire',
    viewingAngle: 'H:140°, V:140°',
    colorDepth: '16 bit',
    voltage: 'AC110V/220V ±10%',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    operatingHumidity: '10%~95%RH',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '5000:1',
    driveMethod: '1:4 Scan',
    lifeTime: 100000,
    certificates: 'CE, BIS, ROHS',
    price: 9500,
    siChannelPrice: 8550,
    resellerPrice: 8075,
  },

  {
    id: 'modular-outdoor-p10.41',
    name: 'Rigel Plus Outdoor P10.41',
    category: 'Modular Series',
    rigelVariant: 'plus',
    enabled: true,
    image: '/products/modular-series/P10.41.png',
    resolution: { width: 96, height: 96 },
    cabinetDimensions: { width: 1000, height: 1000 },
    moduleDimensions: { width: 500, height: 250 },
    moduleResolution: { width: 48, height: 24 },
    moduleQuantity: 8,
    pixelPitch: 10.41,
    pixelDensity: 9216,
    brightness: 6500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-plus/Rigel+Plus+P10.41mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD2727 Copper Wire / Gold Wire',
    viewingAngle: 'H:140°, V:140°',
    colorDepth: '16 bit',
    voltage: 'AC110V/220V ±10%',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    operatingHumidity: '10%~95%RH',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '6000:1',
    driveMethod: '1:2 Scan',
    lifeTime: 100000,
    certificates: 'CE, BIS, ROHS',
    price: 8000,
    siChannelPrice: 7200,
    resellerPrice: 6800,
  },

  // ── Rigel Lite (Modular Series) ──────────────────────────────────────
  {
    id: 'rigel-lite-outdoor-p4',
    name: 'Rigel Lite Outdoor P4',
    category: 'Modular Series',
    rigelVariant: 'lite',
    enabled: true,
    image: '/products/modular-series/P4.png',
    resolution: { width: 240, height: 240 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 480, height: 320 },
    moduleResolution: { width: 120, height: 80 },
    moduleQuantity: 6, // 2×3
    pixelPitch: 4,
    pixelDensity: 62500,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-lite/Rigel+Lite+P4mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD1921',
    viewingAngle: 'H:140°, V:140°',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    storageTempHumidity: '0°C to +35°C',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '5000:1',
    driveMethod: '1/8 Scan',
    lifeTime: 100000,
    certificates: 'CE, FCC, EMC, BIS, ROHS',
    price: 0,
    siChannelPrice: 0,
    resellerPrice: 0,
  },

  {
    id: 'rigel-lite-outdoor-p5',
    name: 'Rigel Lite Outdoor P5',
    category: 'Modular Series',
    rigelVariant: 'lite',
    enabled: true,
    image: '/products/modular-series/P5.png',
    resolution: { width: 192, height: 192 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 480, height: 320 },
    moduleResolution: { width: 96, height: 64 },
    moduleQuantity: 6, // 2×3
    pixelPitch: 5,
    pixelDensity: 40000,
    brightness: 6000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-lite/Rigel+Lite+P5mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD1921',
    viewingAngle: 'H:140°, V:140°',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    storageTempHumidity: '0°C to +35°C',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '6000:1',
    driveMethod: '1/8 Scan',
    lifeTime: 100000,
    certificates: 'CE, FCC, EMC, BIS, ROHS',
    price: 0,
    siChannelPrice: 0,
    resellerPrice: 0,
  },

  {
    id: 'rigel-lite-outdoor-p6.67',
    name: 'Rigel Lite Outdoor P6.67',
    category: 'Modular Series',
    rigelVariant: 'lite',
    enabled: true,
    image: '/products/modular-series/P6.67.png',
    resolution: { width: 144, height: 144 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 480, height: 320 },
    moduleResolution: { width: 72, height: 48 },
    moduleQuantity: 6, // 2×3
    pixelPitch: 6.67,
    pixelDensity: 22500,
    brightness: 6000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-lite/Rigel+Lite+P6.67mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD2727',
    viewingAngle: 'H:140°, V:140°',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    storageTempHumidity: '0°C to +35°C',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '5000:1',
    driveMethod: '1/8 Scan',
    lifeTime: 100000,
    certificates: 'CE, FCC, EMC, BIS, ROHS',
    price: 0,
    siChannelPrice: 0,
    resellerPrice: 0,
  },

  {
    id: 'rigel-lite-outdoor-p8',
    name: 'Rigel Lite Outdoor P8',
    category: 'Modular Series',
    rigelVariant: 'lite',
    enabled: true,
    image: '/products/modular-series/P8.png',
    resolution: { width: 120, height: 120 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 480, height: 320 },
    moduleResolution: { width: 60, height: 40 },
    moduleQuantity: 6, // 2×3
    pixelPitch: 8,
    pixelDensity: 15625,
    brightness: 6000,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-lite/Rigel+Lite+P8mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD2727',
    viewingAngle: 'H:140°, V:140°',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    storageTempHumidity: '0°C to +35°C',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '5000:1',
    driveMethod: '1/4 Scan',
    lifeTime: 100000,
    certificates: 'CE, FCC, EMC, BIS, ROHS',
    price: 0,
    siChannelPrice: 0,
    resellerPrice: 0,
  },

  {
    id: 'rigel-lite-outdoor-p10',
    name: 'Rigel Lite Outdoor P10',
    category: 'Modular Series',
    rigelVariant: 'lite',
    enabled: true,
    image: '/products/modular-series/P10.png',
    resolution: { width: 96, height: 96 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 480, height: 320 },
    moduleResolution: { width: 48, height: 32 },
    moduleQuantity: 6, // 2×3
    pixelPitch: 10,
    pixelDensity: 10000,
    brightness: 6500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 230,
    weightPerCabinet: 25,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/rigel-lite/Rigel+Lite+P10mm.pdf',
    ledType: 'SMD',
    ledConfiguration: 'Nationstar SMD2727',
    viewingAngle: 'H:140°, V:140°',
    serviceAccess: 'Front and Rear',
    ipGrade: 'IP65',
    operatingTemperature: '-20°C to +65°C',
    storageTempHumidity: '0°C to +35°C',
    cabinetMaterial: 'Aluminum profile',
    contrastRatio: '6000:1',
    driveMethod: '1/2 Scan',
    lifeTime: 100000,
    certificates: 'CE, FCC, EMC, BIS, ROHS',
    price: 0,
    siChannelPrice: 0,
    resellerPrice: 0,
  },

  {
    id: 'rigel-cob-p0.9',
    name: 'Sigma Edge P0.9',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor P0.9.png',
    resolution: { width: 640, height: 360 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 160, height: 180 },
    moduleQuantity: 8, // 4x2 modules
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 0.9375,
    pixelDensity: 1137778,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/60',
    connectionTechnology: 'Common Anode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Sigma+Edge-+P0.9mm.pdf',
    price: 60000,
    siChannelPrice: 54000,
    resellerPrice: 51000,
  },

  {
    id: 'rigel-cob-p1.25',
    name: 'Sigma Edge P1.25',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor P1.25.png',
    resolution: { width: 480, height: 270 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 120, height: 135 },
    moduleQuantity: 8, // 4x2 modules
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/60',
    connectionTechnology: 'Common Anode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Sigma+Edge-+P1.25mm.pdf',
    price: 28700,
    siChannelPrice: 25830,
    resellerPrice: 24395,
  },

  {
    id: 'rigel-smd-p1.25-16-9',
    name: 'Astra Edge-I P1.25',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor SMD P1.25.png',
    resolution: { width: 480, height: 270 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 256, height: 128 },
    moduleQuantity: 4, // 2x2 modules
    moduleGrid: { columns: 2, rows: 2 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    brightness: 800,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '160/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-I+P1.25mm.pdf',
    price: 25700,
    siChannelPrice: 23130,
    resellerPrice: 21845,
    cabinetVariations: [
      { label: '600x337.5', cabinetDimensions: { width: 600, height: 337.5 }, resolution: { width: 480, height: 270 }, weightPerCabinet: 8, moduleQuantity: 4 },
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 512, height: 384 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 512, height: 512 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-cob-p1.5',
    name: 'Sigma Edge P1.5',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor P1.56.png',
    resolution: { width: 384, height: 216 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 96, height: 108 },
    moduleQuantity: 8, // 4x2 modules
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 1.5625,
    pixelDensity: 409600,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/48',
    connectionTechnology: 'Common Anode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Sigma+Edge-+P1.56mm.pdf',
    price: 27200,
    siChannelPrice: 24480,
    resellerPrice: 23120,
  },

  {
    id: 'rigel-smd-p1.5-16-9',
    name: 'Astra Edge-I P1.5',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor SMD P1.5.png',
    resolution: { width: 384, height: 216 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 213, height: 107 },
    moduleQuantity: 4, // 2x2 modules
    moduleGrid: { columns: 2, rows: 2 },
    pixelPitch: 1.5,
    pixelDensity: 444000,
    brightness: 800,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '160/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-I+P1.5mm.pdf',
    price: 19900,
    siChannelPrice: 17910,
    resellerPrice: 16915,
    cabinetVariations: [
      { label: '600x337.5', cabinetDimensions: { width: 600, height: 337.5 }, resolution: { width: 384, height: 216 }, weightPerCabinet: 8, moduleQuantity: 4 },
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 426, height: 320 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 426, height: 426 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-smd-p1.8',
    name: 'Astra Edge-I P1.8',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor 1.8.png',
    resolution: { width: 320, height: 180 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 172, height: 86 }, // Calculated from module dimensions and pixel pitch
    moduleQuantity: 4, // 2x2 modules
    moduleGrid: { columns: 2, rows: 2 },
    pixelPitch: 1.86,
    pixelDensity: 284444,
    brightness: 800,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '160/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-I+P1.86mm.pdf',
    price: 18800,
    siChannelPrice: 16920,
    resellerPrice: 15980,
    cabinetVariations: [
      { label: '600x337.5', cabinetDimensions: { width: 600, height: 337.5 }, resolution: { width: 320, height: 180 }, weightPerCabinet: 8, moduleQuantity: 4 },
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 344, height: 258 }, weightPerCabinet: 8, moduleQuantity: 6 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 344, height: 344 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rigel-smd-p2.5',
    name: 'Astra Edge-I P2.5',
    category: 'Edge Series',
    enabled: true,
    image: '/products/rigel-series/Indoor SMD P2.5.png',
    resolution: { width: 256, height: 192 }, // Calculated from module dimensions and pixel pitch
    cabinetDimensions: { width: 640, height: 480 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 }, // Calculated from module dimensions and pixel pitch
    moduleQuantity: 4, // 2x2 modules
    moduleGrid: { columns: 2, rows: 2 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 1000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 8,
    viewingAngle: '160/140',
    colorDepth: '14 bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 100000,
    mtbf: 5000,
    ledChip: 'Nationstar Copper',
    pcb: '4 layer',
    driverIC: 'ICN/MBI',
    powerSupply: 'Chuangliang',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/edge+series/Astra+Edge-I+P2.5mm.pdf',
    price: 11000,
    siChannelPrice: 9900,
    resellerPrice: 9350,
    cabinetVariations: [
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 256, height: 192 }, weightPerCabinet: 8, moduleQuantity: 4 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 256, height: 256 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-indoor-cob-p0.9',
    name: 'Sigma Prime P0.9',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Indoor COB P0.9.png',
    resolution: { width: 640, height: 360 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 160, height: 180 },
    moduleQuantity: 8, // 4x2
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 0.9375,
    pixelDensity: 1137778,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/60',
    connectionTechnology: 'Common Cathode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Sigma+Prime-+P0.9mm.pdf',
    price: 61000,
    siChannelPrice: 54900,
    resellerPrice: 51850,
  },

  {
    id: 'betel-indoor-cob-p1.25',
    name: 'Sigma Prime P1.25',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Indoor COB P1.25.png',
    resolution: { width: 480, height: 270 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 120, height: 135 },
    moduleQuantity: 8, // 4x2
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/60',
    connectionTechnology: 'Common Cathode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Sigma+Prime-+P1.25mm.pdf',
    price: 30900,
    siChannelPrice: 27810,
    resellerPrice: 26265,
  },

  {
    id: 'betel-indoor-cob-p1.5-16-9',
    name: 'Sigma Prime P1.5',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Indoor COB P1.5.png',
    resolution: { width: 384, height: 216 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 150, height: 168.75 },
    moduleResolution: { width: 96, height: 108 },
    moduleQuantity: 8, // 4x2
    moduleGrid: { columns: 4, rows: 2 },
    pixelPitch: 1.5625,
    pixelDensity: 409600,
    brightness: 600,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 420,
    avgPowerConsumption: 150,
    weightPerCabinet: 4.5,
    ledType: 'COB',
    viewingAngle: 'H 170°, V 170°',
    colorDepth: '16 Bit',
    contrastRatio: '10000:1',
    colorTemperature: '5000-12000 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/48',
    connectionTechnology: 'Common Cathode',
    serviceAccess: 'Total Front Access',
    ipGrade: 'IP31',
    operatingTemperature: '0°C~50°C',
    operatingHumidity: '10~90%RH',
    storageTempHumidity: '0°C~60°C, 10~90%RH',
    cabinetMaterial: 'Die-casting Aluminum',
    controller: 'Novastar',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Sigma+Prime-+P1.56mm.pdf',
    price: 28700,
    siChannelPrice: 25830,
    resellerPrice: 24395,
  },

  {
    id: 'betel-indoor-smd-p1.8',
    name: 'Astra Prime-I P1.8',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Indoor SMD P1.8.png',
    resolution: { width: 320, height: 180 },
    cabinetDimensions: { width: 600, height: 337.5 },
    moduleDimensions: { width: 300, height: 168.75 },
    moduleResolution: { width: 172, height: 86 },
    moduleQuantity: 4, // 2x2
    moduleGrid: { columns: 2, rows: 2 },
    pixelPitch: 1.86,
    pixelDensity: 284444,
    brightness: 1000,
    refreshRate: 7680,
    environment: 'indoor',
    maxPowerConsumption: 550,
    avgPowerConsumption: 225,
    weightPerCabinet: 6.5,
    viewingAngle: '160/160',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-I+P1.8mm.pdf',
    price: 20900,
    siChannelPrice: 18810,
    resellerPrice: 17765,
    cabinetVariations: [
      { label: '600x337.5', cabinetDimensions: { width: 600, height: 337.5 }, resolution: { width: 320, height: 180 }, weightPerCabinet: 6.5, moduleQuantity: 4 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 344, height: 344 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-indoor-smd-p2.5',
    name: 'Astra Prime-I P2.5',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Indoor SMD P2.5.png',
    resolution: { width: 256, height: 192 },
    cabinetDimensions: { width: 640, height: 480 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 4, // 2x2
    moduleGrid: { columns: 2, rows: 2 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 1200,
    refreshRate: 7680,
    environment: 'indoor',
    maxPowerConsumption: 600,
    avgPowerConsumption: 275,
    weightPerCabinet: 7.5,
    viewingAngle: '160/160',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-I+P2.5mm.pdf',
    price: 18400,
    siChannelPrice: 16560,
    resellerPrice: 15640,
    cabinetVariations: [
      { label: '640x480', cabinetDimensions: { width: 640, height: 480 }, resolution: { width: 256, height: 192 }, weightPerCabinet: 7.5, moduleQuantity: 4 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 256, height: 256 }, weightPerCabinet: 8, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-outdoor-smd-p2.5',
    name: 'Astra Prime-O P2.5',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Outdoor SMD P2.5.jpg',
    resolution: { width: 256, height: 192 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 6, // 3x2
    moduleGrid: { columns: 3, rows: 2 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 6000,
    refreshRate: 7680,
    environment: 'outdoor',
    maxPowerConsumption: 600,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    viewingAngle: '160/160',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-O+P2.5mm.pdf',
    price: 27900,
    siChannelPrice: 25110,
    resellerPrice: 23715,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 384, height: 384 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 256, height: 256 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-outdoor-smd-p3',
    name: 'Astra Prime-O P3',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Outdoor SMD P3.png',
    resolution: { width: 312, height: 312 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 104, height: 52 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 3.0,
    pixelDensity: 111111,
    brightness: 6500,
    refreshRate: 7680,
    environment: 'outdoor',
    maxPowerConsumption: 450,
    avgPowerConsumption: 200,
    weightPerCabinet: 27,
    viewingAngle: '160/160',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-O+P3mm.pdf',
    price: 18400,
    siChannelPrice: 16560,
    resellerPrice: 15640,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 312, height: 312 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 208, height: 208 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-outdoor-smd-p4',
    name: 'Astra Prime-O P4',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Outdoor SMD P4.png',
    resolution: { width: 240, height: 240 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 4.0,
    pixelDensity: 62500,
    brightness: 6500,
    refreshRate: 7680,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    viewingAngle: '160/160',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-O+P4mm.pdf',
    price: 15400,
    siChannelPrice: 13860,
    resellerPrice: 13090,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 240, height: 240 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 160, height: 160 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-outdoor-smd-p6.6',
    name: 'Astra Prime-O P6.6',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Outdoor SMD P6.6.png',
    resolution: { width: 144, height: 144 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 48, height: 24 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 6.6,
    pixelDensity: 23104,
    brightness: 8000,
    refreshRate: 7680,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    viewingAngle: 'Customised',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-O+P6.6mm.pdf',
    price: 9400,
    siChannelPrice: 8460,
    resellerPrice: 7990,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 144, height: 144 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 96, height: 96 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'betel-outdoor-smd-p10',
    name: 'Astra Prime-O P10',
    category: 'Prime Series',
    enabled: true,
    image: '/products/Betel-series/Betelgeuse Series Outdoor SMD P10.png',
    resolution: { width: 96, height: 96 },
    cabinetDimensions: { width: 960, height: 960 },
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 32, height: 16 },
    moduleQuantity: 18, // 3x6
    moduleGrid: { columns: 3, rows: 6 },
    pixelPitch: 10.0,
    pixelDensity: 10000,
    brightness: 8500,
    refreshRate: 7680,
    environment: 'outdoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 27,
    viewingAngle: 'Customised',
    colorDepth: '16 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 100000,
    mtbf: 10000,
    ledChip: 'Nation star Gold Bond',
    pcb: '6 Layer',
    driverIC: 'MBI/ICN/TI',
    powerSupply: 'Meanwell',
    controller: 'Nova',
    cabinetMaterial: 'Die Cast',
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Astra+Prime-O+P10mm.pdf',
    price: 8100,
    siChannelPrice: 7290,
    resellerPrice: 6885,
    cabinetVariations: [
      { label: '960x960', cabinetDimensions: { width: 960, height: 960 }, resolution: { width: 96, height: 96 }, weightPerCabinet: 27, moduleQuantity: 18 },
      { label: '640x640', cabinetDimensions: { width: 640, height: 640 }, resolution: { width: 64, height: 64 }, weightPerCabinet: 20, moduleQuantity: 8 }
    ],
  },

  {
    id: 'rental-indoor-p2.6',
    name: 'Bellatrix Series-I P2.6',
    category: 'Rental Series',
    enabled: true,
    image: '/products/rental-series/P2.6.png',
    resolution: { width: 192, height: 192 },
    cabinetDimensions: { width: 500, height: 500 },
    moduleDimensions: { width: 250, height: 250 },
    moduleResolution: { width: 96, height: 96 },
    moduleQuantity: 4, // 2x2
    pixelPitch: 2.6,
    pixelDensity: 147456,
    brightness: 900,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 280,
    avgPowerConsumption: 120,
    weightPerCabinet: 8.5,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Bellatrix/Bellatrix-I+P2.6mm.pdf',
    viewingAngle: 'H 140° ， V 140°',
    ledType: 'Black SMD 2121 / Black SMD 2020',
    contrastRatio: '10000:1',
    colorTemperature: '3200-9300 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/32 1/16',
    serviceAccess: 'FRONT',
    ipGrade: 'IP31/43',
    operatingTemperature: '0℃～50℃, 10～90%RH',
    storageTempHumidity: '0℃～60℃, 10～90%RH',
    controller: 'Novastar',
    cabinetMaterial: 'Die-casting Aluminum',
    certificates: 'CE/BIS/ISO/ROHS/MII',
    prices: {
      cabinet: { endCustomer: 34375, siChannel: 32353, reseller: 30556 },
      curveLock: { endCustomer: 1700, siChannel: 1600, reseller: 1500 }
    }
  },

  {
    id: 'rental-indoor-p2.97',
    name: 'Bellatrix Series-I P2.97',
    category: 'Rental Series',
    enabled: true,
    image: '/products/rental-series/P2.97.png',
    resolution: { width: 168, height: 168 },
    cabinetDimensions: { width: 500, height: 500 },
    moduleDimensions: { width: 250, height: 250 },
    moduleResolution: { width: 84, height: 84 },
    moduleQuantity: 4, // 2x2
    pixelPitch: 2.97,
    pixelDensity: 112896,
    brightness: 900,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 280,
    avgPowerConsumption: 120,
    weightPerCabinet: 8.5,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Bellatrix/Bellatrix-I+P2.97mm.pdf',
    viewingAngle: 'H 140° ， V 140°',
    ledType: 'Black SMD 2121 / Black SMD 2020',
    contrastRatio: '10000:1',
    colorTemperature: '3200-9300 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/21',
    serviceAccess: 'FRONT',
    ipGrade: 'IP31/43',
    operatingTemperature: '0 ℃～ 50 ℃ , 10 ～ 90%RH',
    storageTempHumidity: '0℃～60℃, 10～90%RH',
    controller: 'Novastar',
    cabinetMaterial: 'Die-casting Aluminum',
    certificates: 'CE/BIS/ISO/ROHS/MII',
    prices: {
      cabinet: { endCustomer: 28500, siChannel: 26824, reseller: 25333 },
      curveLock: { endCustomer: 1700, siChannel: 1600, reseller: 1500 }
    }
  },

  {
    id: 'rental-outdoor-p3.8',
    name: 'Bellatrix Series-O P3.8',
    category: 'Rental Series',
    enabled: true,
    image: '/products/rental-series/P3.84.png',
    resolution: { width: 150, height: 150 },
    cabinetDimensions: { width: 576, height: 576 },
    moduleDimensions: { width: 288, height: 288 },
    moduleResolution: { width: 75, height: 75 },
    moduleQuantity: 4, // 2x2
    pixelPitch: 3.84,
    pixelDensity: 67600,
    brightness: 3500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 500,
    avgPowerConsumption: 220,
    weightPerCabinet: 8.5,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Bellatrix/Bellatrix-O+P3.8mm.pdf',
    viewingAngle: 'H 140° ， V 140°',
    ledType: 'Atenti SMD LED',
    contrastRatio: '10000:1',
    colorTemperature: '3200-9300 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/25',
    serviceAccess: 'FRONT/REAR',
    ipGrade: 'IP65/54',
    operatingTemperature: '-20℃～ 50 ℃ , 10 ～ 90%RH',
    storageTempHumidity: '-20℃～ 60 ℃ , 10 ～ 90%RH',
    controller: 'Novastar',
    cabinetMaterial: 'Die-casting Aluminum',
    certificates: 'CE/BIS/ISO/ROHS/MII',
    prices: {
      cabinet: { endCustomer: 31188, siChannel: 29353, reseller: 27722 },
      curveLock: { endCustomer: 2100, siChannel: 1900, reseller: 1800 }
    }
  },

  {
    id: 'rental-outdoor-p4.8',
    name: 'Bellatrix Series-O P4.8',
    category: 'Rental Series',
    enabled: true,
    image: '/products/rental-series/P4.81.png',
    resolution: { width: 120, height: 120 },
    cabinetDimensions: { width: 576, height: 576 },
    moduleDimensions: { width: 288, height: 288 },
    moduleResolution: { width: 60, height: 60 },
    moduleQuantity: 4, // 2x2
    pixelPitch: 4.81,
    pixelDensity: 43403,
    brightness: 3500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 500,
    avgPowerConsumption: 220,
    weightPerCabinet: 8.5,
    pdf: 'https://orion-led-quotations.s3.ap-south-1.amazonaws.com/Product+specs+pdf+/Bellatrix/Bellatrix-O+P4.8mm.pdf',
    viewingAngle: 'H 140° ， V 140°',
    ledType: 'Atenti SMD LED',
    contrastRatio: '10000:1',
    colorTemperature: '3200-9300 Optional',
    frameChangingFrequently: '50/60',
    driveMethod: '1/15',
    serviceAccess: 'FRONT/REAR',
    ipGrade: 'IP65/54',
    operatingTemperature: '-20 ℃～ 50 ℃ , 10 ～ 90%RH',
    storageTempHumidity: '-20 ℃～ 60 ℃ , 10 ～ 90%RH',
    controller: 'Novastar',
    cabinetMaterial: 'Die-casting Aluminum',
    certificates: 'CE/BIS/ISO/ROHS/MII',
    prices: {
      cabinet: { endCustomer: 28125, siChannel: 26471, reseller: 25000 },
      curveLock: { endCustomer: 2100, siChannel: 1900, reseller: 1800 }
    }
  },

  {
    id: 'jumbo-outdoor-p6',
    name: 'Mega Series Outdoor P6',
    category: 'Jumbo Series',
    enabled: JUMBO_SERIES_ACTIVE,
    image: '/products/jumbo-series/Jumbo Series Outdoor P6.png',
    resolution: { width: 32, height: 32 }, // Per module: 32×columns × 32×rows (192mm/6mm = 32 px)
    cabinetDimensions: { width: 2109, height: 1536 },
    moduleDimensions: { width: 192, height: 192 }, // 192×192mm per Orion P6 spec
    moduleResolution: { width: 32, height: 32 },
    moduleQuantity: 88,
    moduleGrid: { columns: 11, rows: 8 },
    pixelPitch: 6,
    pixelDensity: 27776,
    brightness: 5500,
    refreshRate: 1920,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 350,
    weightPerCabinet: 27,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/jumbo-series/4_Jumbo+Series+Outdoor+SMD+P6.pdf',
    ledType: 'SMD',
    ledConfiguration: '1R+1G+1B',
    driverIC: 'ICN/SM/MBI or Equivalent',
    powerSupply: 'Rong',
    minViewDistance: '6m',
    viewingAngle: 'H:140°, V:140° @ 50% brightness',
    brightnessAdjustment: 'Manual, 256 level; Auto, 8 levels',
    colorDepth: '16 bit/color',
    availableColorPalette: '281 trillion colors',
    operatingTemperature: '-10°C ~ +65°C',
    operatingHumidity: '10%~95%RH',
    lifeTime: 100000,
    voltage: 'AC110V/220V/380V ±10%',
    inputPowerFrequency: '>50/60Hz',
    defectRate: '≤0.0001',
    dataTransferMethod: 'RJ45',
    controlMethod: 'Synchronized with computer monitor',
    driveMethod: 'Constant current driving, 1/5',
    mtbf: 10000,
    controlDistance: 'CAT5: 120 m (no repeating); Optical fiber transmission: 500m - 10km',
    serviceAccess: 'Rear Access',
    ipGrade: 'IP65',
    certificates: 'CE, ROHS, ISO, BIS',
    dimensionConstraints: {
      series: 'Jumbo',
      moduleWidth: 192,
      moduleHeight: 192,
      minWidth: 960,
      minHeight: 960,
      maxWidth: 3840,
      maxHeight: 2496,
      validationMessage: 'For Jumbo Series Outdoor P6, screen size must be between 960x960mm and 3840x2496mm and in multiples of 192x192 module size.',
    },
    price: 6600,
    siChannelPrice: 6000,
    resellerPrice: 5600,
  },

  {
    id: 'jumbo-outdoor-p4',
    name: 'Mega Series Outdoor P4',
    category: 'Jumbo Series',
    enabled: JUMBO_SERIES_ACTIVE,
    image: '/products/jumbo-series/Jumbo Series Outdoor P4.png',
    resolution: { width: 80, height: 40 }, // Per module: (Column × 80) × (Row × 40)
    cabinetDimensions: { width: 2237, height: 1439 }, // Fixed display size: 7.34ft x 4.72ft
    moduleDimensions: { width: 320, height: 160 }, // Fixed module size: 320x160mm
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 63, // 7 columns x 9 rows
    moduleGrid: { columns: 7, rows: 9 },
    pixelPitch: 4,
    pixelDensity: 62500,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 350,
    weightPerCabinet: 27,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/jumbo-series/3_Jumbo+Series+Outdoor+SMD+P4.pdf',
    ledType: 'SMD',
    ledConfiguration: '1R+1PG+1PB',
    driverIC: 'ICN/SM/MBI or Equivalent',
    powerSupply: 'Rong',
    minViewDistance: '4m',
    viewingAngle: 'H:160°, V:160° @ 50% brightness',
    brightnessAdjustment: 'Manual, 256 level; Auto, 8 levels',
    colorDepth: '16 bit/color',
    availableColorPalette: '281 trillion colors',
    operatingTemperature: '-10°C ~ +65°C',
    operatingHumidity: '10%~95%RH',
    lifeTime: 100000,
    voltage: 'AC110V/220V/380V ±10%',
    inputPowerFrequency: '>50/60Hz',
    defectRate: '≤0.0001',
    dataTransferMethod: 'RJ45',
    controlMethod: 'Synchronized with computer monitor',
    driveMethod: 'Constant current driving, 1/5',
    mtbf: 10000,
    controlDistance: 'CAT5: 120 m (no repeating); Optical fiber transmission: 500m - 10km',
    serviceAccess: 'Rear Access',
    ipGrade: 'IP65',
    certificates: 'CE, ROHS, ISO, BIS',
    dimensionConstraints: {
      series: 'Jumbo',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 960,
      minHeight: 960,
      maxWidth: 3840,
      maxHeight: 2400,
      validationMessage: 'For Jumbo Series Outdoor P4, screen size must be between 960x960mm and 3840x2400mm and in multiples of 320x160 module size.',
    },
    price: 7900,
    siChannelPrice: 7200,
    resellerPrice: 6800,
  },

  {
    id: 'jumbo-outdoor-p3',
    name: 'Mega Series Outdoor P3',
    category: 'Jumbo Series',
    enabled: JUMBO_SERIES_ACTIVE,
    image: '/products/jumbo-series/Jumbo Series Outdoor P3.png',
    resolution: { width: 64, height: 64 }, // Per module: (Column × 64) × (Row × 64)
    cabinetDimensions: { width: 2109, height: 1536 }, // Fixed display size: 6.92ft x 5.04ft
    moduleDimensions: { width: 192, height: 192 }, // Fixed module size: 192x192mm
    moduleResolution: { width: 64, height: 64 },
    moduleQuantity: 88, // 11 columns x 8 rows
    moduleGrid: { columns: 11, rows: 8 },
    pixelPitch: 3,
    pixelDensity: 111000,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 350,
    weightPerCabinet: 27,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/jumbo-series/2_Jumbo+Series+Outdoor+SMD+P3.pdf',
    ledType: 'SMD',
    ledConfiguration: '1R+1PG+1PB',
    driverIC: 'ICN/SM/MBI or Equivalent',
    powerSupply: 'Rong',
    minViewDistance: '3m',
    viewingAngle: 'H:160°, V:160° @ 50% brightness',
    brightnessAdjustment: 'Manual, 256 level; Auto, 8 levels',
    colorDepth: '16 bit/color',
    availableColorPalette: '281 trillion colors',
    operatingTemperature: '-10°C ~ +65°C',
    operatingHumidity: '10%~95%RH',
    lifeTime: 100000,
    voltage: 'AC110V/220V/380V ±10%',
    inputPowerFrequency: '>50/60Hz',
    defectRate: '≤0.0001',
    dataTransferMethod: 'RJ45',
    controlMethod: 'Synchronized with computer monitor',
    driveMethod: 'Constant current driving, 1/5',
    mtbf: 10000,
    controlDistance: 'CAT5: 120 m (no repeating); Optical fiber transmission: 500m - 10km',
    serviceAccess: 'Rear Access',
    ipGrade: 'IP65',
    certificates: 'CE, ROHS, ISO, BIS',
    dimensionConstraints: {
      series: 'Jumbo',
      moduleWidth: 192,
      moduleHeight: 192,
      minWidth: 960,
      minHeight: 960,
      maxWidth: 3840,
      maxHeight: 2496,
      validationMessage: 'For Jumbo Series Outdoor P3, screen size must be between 960x960mm and 3840x2496mm and in multiples of 192x192 module size.',
    },
    price: 9400,
    siChannelPrice: 8500,
    resellerPrice: 8000,
  },

  {
    id: 'jumbo-indoor-p2.5',
    name: 'Mega Series Outdoor P2.5',
    category: 'Jumbo Series',
    enabled: JUMBO_SERIES_ACTIVE,
    image: '/products/jumbo-series/Jumbo Series Outdoor P2.5.png',
    resolution: { width: 128, height: 64 }, // Per module: (Column × 128) × (Row × 64)
    cabinetDimensions: { width: 2237, height: 1439 }, // Fixed display size: 7.34ft x 4.72ft
    moduleDimensions: { width: 320, height: 160 }, // Fixed module size: 320x160mm
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 63, // 7 columns x 9 rows
    moduleGrid: { columns: 7, rows: 9 },
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 4500,
    refreshRate: 3840,
    environment: 'outdoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 300,
    weightPerCabinet: 27,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/jumbo-series/1_Jumbo+Series+Outdoor+SMD+P2.5.pdf',
    ledType: 'SMD',
    ledConfiguration: '1R+1PG+1PB',
    driverIC: 'ICN/SM/MBI or Equivalent',
    powerSupply: 'Rong',
    minViewDistance: '2.5m',
    viewingAngle: 'H:160°, V:160° @ 50% brightness',
    brightnessAdjustment: 'Manual, 256 level; Auto, 8 levels',
    colorDepth: '16 bit/color',
    availableColorPalette: '281 trillion colors',
    operatingTemperature: '-10°C ~ +65°C',
    operatingHumidity: '10%~95%RH',
    lifeTime: 100000,
    voltage: 'AC110V/220V/380V ±10%',
    inputPowerFrequency: '>50/60Hz',
    defectRate: '≤0.0001',
    dataTransferMethod: 'RJ45',
    controlMethod: 'Synchronized with computer monitor',
    driveMethod: 'Constant current driving, 1/5',
    mtbf: 10000,
    controlDistance: 'CAT5: 120 m (no repeating); Optical fiber transmission: 500m - 10km',
    serviceAccess: 'Rear Access',
    ipGrade: 'IP65',
    certificates: 'CE, ROHS, ISO, BIS',
    dimensionConstraints: {
      series: 'Jumbo',
      moduleWidth: 320,
      moduleHeight: 160,
      minWidth: 960,
      minHeight: 960,
      maxWidth: 3840,
      maxHeight: 2400,
      validationMessage: 'For Jumbo Series Outdoor P2.5, screen size must be between 960x960mm and 3840x2400mm and in multiples of 320x160 module size.',
    },
    price: 12500,
    siChannelPrice: 11300,
    resellerPrice: 10600,
  },

  {
    id: 'transparent-front-glass-p3.91',
    name: 'Crystal Blaze P3.91',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P3.91.avif',
    resolution: {
      width: 64,
      height: 300
    },
    price: 44300,
    siChannelPrice: 40100,
    resellerPrice: 37600,
    cabinetDimensions: {
      width: 250,
      height: 1200
    },
    moduleDimensions: {
      width: 250,
      height: 1200
    },
    moduleResolution: {
      width: 64,
      height: 300
    },
    moduleQuantity: 1,
    pixelPitch: 3.91,
    pixelDensity: 65536,
    brightness: 5000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 400,
    avgPowerConsumption: 150,
    weightPerCabinet: 8,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Front+Glass+P3.91.pdf',
    transparency: 90,
    scanMode: 'Static',
    pixelComposition: 'SMD2121'
  },
  {
    id: 'transparent-front-glass-p6.25',
    name: 'Crystal Blaze P6.25',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P6.25.avif',
    resolution: {
      width: 40,
      height: 188
    },
    price: 24300,
    siChannelPrice: 22000,
    resellerPrice: 20600,
    cabinetDimensions: {
      width: 250,
      height: 1200
    },
    moduleDimensions: {
      width: 250,
      height: 1200
    },
    moduleResolution: {
      width: 40,
      height: 188
    },
    moduleQuantity: 1,
    pixelPitch: 6.25,
    pixelDensity: 25600,
    brightness: 5000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 350,
    avgPowerConsumption: 120,
    weightPerCabinet: 8,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Front+Glass+P6.25.pdf',
    transparency: 95,
    scanMode: 'Static',
    pixelComposition: 'SMD2121'
  },

  {
    id: 'transparent-behind-glass-p6.5',
    name: 'Crystal Elite P6.5',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/image (3).png',
    resolution: {
      width: 32,
      height: 148
    },
    price: 22900,
    siChannelPrice: 20800,
    resellerPrice: 19500,
    cabinetDimensions: {
      width: 208,
      height: 1000
    },
    moduleDimensions: {
      width: 208,
      height: 1000
    },
    moduleResolution: {
      width: 32,
      height: 148
    },
    moduleQuantity: 1,
    pixelPitch: 6.5,
    pixelDensity: 23716,
    brightness: 5500,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 400,
    avgPowerConsumption: 150,
    weightPerCabinet: 6,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Behind+Glass+P6.5.pdf',
    transparency: 55,
    scanMode: 'Static',
    pixelComposition: 'SMD2020'
  },

  {
    id: 'transparent-rollable-film-p5',
    name: 'Crystal Flex P5',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P5-5mm.png',
    resolution: {
      width: 64,
      height: 200
    },
    price: 40400,
    siChannelPrice: 36700,
    resellerPrice: 34400,
    cabinetDimensions: {
      width: 320,
      height: 1000
    },
    moduleDimensions: {
      width: 320,
      height: 1000
    },
    moduleResolution: {
      width: 64,
      height: 200
    },
    moduleQuantity: 1,
    pixelPitch: 5.0,
    pixelDensity: 40000,
    brightness: 2000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 300,
    avgPowerConsumption: 100,
    weightPerCabinet: 2,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Rollable+P5.pdf',
    transparency: 85,
    scanMode: 'Static',
    pixelComposition: 'SMD1313'
  },
  {
    id: 'transparent-rollable-film-p6.25',
    name: 'Crystal Flex P6.25',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P6.25-6.25mm.png',
    resolution: {
      width: 64,
      height: 160
    },
    price: 30900,
    siChannelPrice: 28000,
    resellerPrice: 26300,
    cabinetDimensions: {
      width: 400,
      height: 1000
    },
    moduleDimensions: {
      width: 400,
      height: 1000
    },
    moduleResolution: {
      width: 64,
      height: 160
    },
    moduleQuantity: 1,
    pixelPitch: 6.25,
    pixelDensity: 25600,
    brightness: 2000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 280,
    avgPowerConsumption: 90,
    weightPerCabinet: 2,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Rollable+P6.25.pdf',
    transparency: 90,
    scanMode: 'Static',
    pixelComposition: 'SMD1313'
  },
  {
    id: 'transparent-rollable-film-p8',
    name: 'Crystal Flex P8',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P5-5mm.png',
    resolution: {
      width: 50,
      height: 125
    },
    price: 24300,
    siChannelPrice: 22000,
    resellerPrice: 20600,
    cabinetDimensions: {
      width: 400,
      height: 1000
    },
    moduleDimensions: {
      width: 400,
      height: 1000
    },
    moduleResolution: {
      width: 50,
      height: 125
    },
    moduleQuantity: 1,
    pixelPitch: 8.0,
    pixelDensity: 16500,
    brightness: 2000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 250,
    avgPowerConsumption: 80,
    weightPerCabinet: 2,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Rollable+P8.pdf',
    transparency: 92,
    scanMode: 'Static',
    pixelComposition: 'SMD1313'
  },
  {
    id: 'transparent-rollable-film-p10',
    name: 'Crystal Flex P10',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P6.25-6.25mm.png',
    resolution: {
      width: 40,
      height: 100
    },
    price: 19900,
    siChannelPrice: 18000,
    resellerPrice: 16900,
    cabinetDimensions: {
      width: 400,
      height: 1000
    },
    moduleDimensions: {
      width: 400,
      height: 1000
    },
    moduleResolution: {
      width: 40,
      height: 100
    },
    moduleQuantity: 1,
    pixelPitch: 10.0,
    pixelDensity: 10000,
    brightness: 2000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 220,
    avgPowerConsumption: 70,
    weightPerCabinet: 2,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Rollable+P10.pdf',
    transparency: 93,
    scanMode: 'Static',
    pixelComposition: 'SMD1313'
  },
  {
    id: 'transparent-rollable-film-p15',
    name: 'Crystal Flex P15',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P5-5mm.png',
    resolution: {
      width: 26,
      height: 66
    },
    price: 15400,
    siChannelPrice: 14000,
    resellerPrice: 13100,
    cabinetDimensions: {
      width: 390,
      height: 990
    },
    moduleDimensions: {
      width: 390,
      height: 990
    },
    moduleResolution: {
      width: 26,
      height: 66
    },
    moduleQuantity: 1,
    pixelPitch: 15.0,
    pixelDensity: 4356,
    brightness: 2000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 180,
    avgPowerConsumption: 60,
    weightPerCabinet: 2,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Rollable+P15.pdf',
    transparency: 94,
    scanMode: 'Static',
    pixelComposition: 'SMD1313'
  },
  {
    id: 'transparent-rollable-film-p20',
    name: 'Crystal Flex P20',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/P6.25-6.25mm.png',
    resolution: {
      width: 20,
      height: 50
    },
    price: 13200,
    siChannelPrice: 12000,
    resellerPrice: 11300,
    cabinetDimensions: {
      width: 400,
      height: 1000
    },
    moduleDimensions: {
      width: 400,
      height: 1000
    },
    moduleResolution: {
      width: 20,
      height: 50
    },
    moduleQuantity: 1,
    pixelPitch: 20.0,
    pixelDensity: 2500,
    brightness: 2000,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 150,
    avgPowerConsumption: 50,
    weightPerCabinet: 2,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Transparent+Rollable+P20.pdf',
    transparency: 95,
    scanMode: 'Static',
    pixelComposition: 'SMD1313'
  },

  {
    id: 'transparent-standard-p3.91-7.82-256x128',
    name: 'Crystal Go P3.91-7.82',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/Crystal Go.png',
    resolution: {
      width: 256,
      height: 128
    },
    price: 11200,
    siChannelPrice: 10100,
    resellerPrice: 9500,
    cabinetDimensions: {
      width: 1000,
      height: 1000
    },
    moduleDimensions: {
      width: 1000,
      height: 1000
    },
    moduleResolution: {
      width: 256,
      height: 128
    },
    moduleQuantity: 1,
    pixelPitch: 3.91, // Average of P3.91-7.82
    pixelDensity: 32768,
    brightness: 4500,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 350,
    avgPowerConsumption: 120,
    weightPerCabinet: 15,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Standard+Transparent+P3.91-7.82.pdf',
    transparency: 55,
    scanMode: 'Static',
    pixelComposition: 'SMD1921'
  },
  {
    id: 'transparent-standard-p3.91-7.82-256x64',
    name: 'Crystal Go P3.91-7.82',
    category: 'Transparent Series',
    enabled: true,
    image: '/products/transparent-series/Crystal GO Lite.png',
    resolution: {
      width: 256,
      height: 64
    },
    price: 11200,
    siChannelPrice: 10100,
    resellerPrice: 9500,
    cabinetDimensions: {
      width: 1000,
      height: 500
    },
    moduleDimensions: {
      width: 1000,
      height: 500
    },
    moduleResolution: {
      width: 256,
      height: 64
    },
    moduleQuantity: 1,
    pixelPitch: 3.91, // Average of P3.91-7.82
    pixelDensity: 32768,
    brightness: 4500,
    refreshRate: 3840,
    environment: 'indoor',
    maxPowerConsumption: 350,
    avgPowerConsumption: 120,
    weightPerCabinet: 15,
    pdf: 'https://origin-cms1.s3.ap-south-1.amazonaws.com/products-pdfs/transparent-series/Standard+Transparent+P3.91-7.82.pdf',
    transparency: 55,
    scanMode: 'Static',
    pixelComposition: 'SMD1921'
  },

  // Module/ Grid Series (Indoor, 320×160mm module only — no cabinet size)
  {
    id: 'orion-module-grid-p18',
    name: 'Module/ Grid Series SMD P1.8',
    category: 'Module/ Grid Series',
    enabled: true,
    image: '/products/module series/Module Grid Series P1.8.png.jpg',
    resolution: { width: 172, height: 86 }, // 320/1.86, 160/1.86
    cabinetDimensions: { width: 320, height: 160 }, // Module size only (no separate cabinet)
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 172, height: 86 },
    moduleQuantity: 1,
    pixelPitch: 1.86,
    pixelDensity: 284444,
    brightness: 800,
    refreshRate: 3840,
    environment: 'Indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 225,
    weightPerCabinet: 0, // Cabinet size not included for this series
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 50000,
    mtbf: 5000,
    price: 9640,        // per ft² — End Customer
    siChannelPrice: 8740,  // per ft² — SI/Channel
    resellerPrice: 8194,   // per ft² — Reseller
    prices: {
      cabinet: { endCustomer: 9640, siChannel: 8740, reseller: 8194 }
    }
  },
  {
    id: 'orion-module-grid-p25',
    name: 'Module/ Grid Series SMD P2.5',
    category: 'Module/ Grid Series',
    enabled: true,
    image: '/products/module series/Module Grid Series P2.5.png.jpg',
    resolution: { width: 128, height: 64 }, // 320/2.5, 160/2.5
    cabinetDimensions: { width: 320, height: 160 }, // Module size only (no separate cabinet)
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 128, height: 64 },
    moduleQuantity: 1,
    pixelPitch: 2.5,
    pixelDensity: 160000,
    brightness: 900,
    refreshRate: 3840,
    environment: 'Indoor',
    maxPowerConsumption: 650,
    avgPowerConsumption: 350,
    weightPerCabinet: 0, // Cabinet size not included for this series
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Front',
    ipGrade: '31/31',
    operatingTemperature: '0°C~+50°C',
    operatingHumidity: '0-50%',
    lifeTime: 50000,
    mtbf: 5000,
    price: 7279,        // per ft² — End Customer
    siChannelPrice: 6600,  // per ft² — SI/Channel
    resellerPrice: 6188,   // per ft² — Reseller
    prices: {
      cabinet: { endCustomer: 7279, siChannel: 6600, reseller: 6188 }
    }
  },
  {
    id: 'orion-module-grid-p4',
    name: 'Module/ Grid Series SMD P4',
    category: 'Module/ Grid Series',
    enabled: true,
    image: '/products/module series/Module Grid Series P4.png.jpg',
    resolution: { width: 80, height: 40 }, // 320/4, 160/4
    cabinetDimensions: { width: 320, height: 160 }, // Module size only (no separate cabinet)
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 80, height: 40 },
    moduleQuantity: 1,
    pixelPitch: 4,
    pixelDensity: 62500,
    brightness: 5000,
    refreshRate: 3840,
    environment: 'Indoor',
    maxPowerConsumption: 700,
    avgPowerConsumption: 400,
    weightPerCabinet: 0, // Cabinet size not included for this series
    viewingAngle: '140/140',
    colorDepth: '14 Bit',
    voltage: '220v',
    serviceAccess: 'Rear',
    ipGrade: '65/54',
    operatingTemperature: '-10°C~+65°C',
    operatingHumidity: '0-99%',
    lifeTime: 50000,
    mtbf: 5000,
    price: 5735,        // per ft² — End Customer
    siChannelPrice: 5200,  // per ft² — SI/Channel
    resellerPrice: 4875,   // per ft² — Reseller
    prices: {
      cabinet: { endCustomer: 5735, siChannel: 5200, reseller: 4875 }
    }
  },

  {
    id: "nexa-p125-108",
    name: "Nexa Series 108\" P1.25 (Astra)",
    category: "Nexa-S",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/P1.25.png",
    resolution: { width: 1920, height: 1080 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    cabinetDimensions: { width: 2391, height: 1345 }, // ~108" diagonal 16:9
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 256, height: 128 },
    moduleQuantity: 1,
    brightness: 600,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 760,
    avgPowerConsumption: 250,
    weightPerCabinet: 80,
    ledType: "SMD1010",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/45",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 1670000,
    siChannelPrice: 1590000,
    resellerPrice: 1500000,
    prices: {
      cabinet: { endCustomer: 1670000, siChannel: 1590000, reseller: 1500000 }
    }
  },

  {
    id: "nexa-p156-135",
    name: "Nexa Series 135\" P1.56 (Astra)",
    category: "Nexa-S",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/WhatsApp Image 2026-05-09 at 11.55.14 (1).jpeg",
    resolution: { width: 1920, height: 1080 },
    pixelPitch: 1.5625,
    pixelDensity: 409600,
    cabinetDimensions: { width: 2989, height: 1681 }, // ~135" diagonal 16:9
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 204, height: 102 },
    moduleQuantity: 1,
    brightness: 600,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 1200,
    avgPowerConsumption: 400,
    weightPerCabinet: 120,
    ledType: "SMD1010",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/48",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 1670000,
    siChannelPrice: 1590000,
    resellerPrice: 1500000,
    prices: {
      cabinet: { endCustomer: 1670000, siChannel: 1590000, reseller: 1500000 }
    }
  },
  {
    id: "nexa-p1875-162",
    name: "Nexa Series 162\" P1.875 (Astra)",
    category: "Nexa-S",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/WhatsApp Image 2026-05-09 at 11.55.14.jpeg",
    resolution: { width: 1920, height: 1080 },
    pixelPitch: 1.875,
    pixelDensity: 284444,
    cabinetDimensions: { width: 3587, height: 2018 }, // ~162" diagonal 16:9
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 170, height: 85 },
    moduleQuantity: 1,
    brightness: 600,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 1800,
    avgPowerConsumption: 600,
    weightPerCabinet: 160,
    ledType: "SMD1010",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/45",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 1960000,
    siChannelPrice: 1860000,
    resellerPrice: 1760000,
    prices: {
      cabinet: { endCustomer: 1960000, siChannel: 1860000, reseller: 1760000 }
    }
  },
  {
    id: "nexa-p09-162-cob",
    name: "Nexa Series 162\" P0.9 (Sigma)",
    category: "Nexa-C",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/P0.9mm.png",
    resolution: { width: 3840, height: 2160 },
    pixelPitch: 0.9375,
    pixelDensity: 1137778,
    cabinetDimensions: { width: 3587, height: 2018 }, // ~162" diagonal 16:9
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 341, height: 170 },
    moduleQuantity: 1,
    brightness: 600,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 1800,
    avgPowerConsumption: 600,
    weightPerCabinet: 160,
    ledType: "COB Black LED",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/60",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 5440000,
    siChannelPrice: 5170000,
    resellerPrice: 4900000,
    prices: {
      cabinet: { endCustomer: 5440000, siChannel: 5170000, reseller: 4900000 }
    }
  },
  {
    id: "nexa-c-p125",
    name: "Nexa Series 108\" P1.25 (Sigma)",
    category: "Nexa-C",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/P1.25 108 inch.png",
    resolution: { width: 1920, height: 1080 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    cabinetDimensions: { width: 2400, height: 1350 }, // 16:9 aspect ratio
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 256, height: 128 },
    moduleQuantity: 1,
    brightness: 600,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 760,
    avgPowerConsumption: 400,
    weightPerCabinet: 120,
    ledType: "COB LED",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/48",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 1550000,
    siChannelPrice: 1472500,
    resellerPrice: 1395000,
    prices: {
      cabinet: { endCustomer: 1550000, siChannel: 1472500, reseller: 1395000 }
    }
  },
  {
    id: "nexa-c-p156",
    name: "Nexa Series 135\" P1.56 (Sigma)",
    category: "Nexa-C",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/P1.56.png",
    resolution: { width: 1920, height: 1080 },
    pixelPitch: 1.5625,
    pixelDensity: 409600,
    cabinetDimensions: { width: 2988, height: 1683 }, // 16:9 aspect ratio
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 192, height: 96 },
    moduleQuantity: 1,
    brightness: 1250,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 1200,
    avgPowerConsumption: 550,
    weightPerCabinet: 160,
    ledType: "COB LED",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/45",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 1950000,
    siChannelPrice: 1852500,
    resellerPrice: 1755000,
    prices: {
      cabinet: { endCustomer: 1950000, siChannel: 1852500, reseller: 1755000 }
    }
  },
  {
    id: "nexa-c-p1875",
    name: "Nexa Series 162\" P1.875 (Sigma)",
    category: "Nexa-C",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/P1.87inch.png",
    resolution: { width: 1920, height: 1080 },
    pixelPitch: 1.875,
    pixelDensity: 284444,
    cabinetDimensions: { width: 3585, height: 2020 }, // 16:9 aspect ratio
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 170, height: 85 },
    moduleQuantity: 1,
    brightness: 1250,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 1800,
    avgPowerConsumption: 750,
    weightPerCabinet: 200,
    ledType: "COB LED",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/45",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 2450000,
    siChannelPrice: 2327500,
    resellerPrice: 2205000,
    prices: {
      cabinet: { endCustomer: 2450000, siChannel: 2327500, reseller: 2205000 }
    }
  },
  {
    id: "nexa-c-p125-216",
    name: "Nexa Series 216\" P1.25 (Sigma)",
    category: "Nexa-C",
    isFixed: true,
    enabled: true,
    image: "/products/nexa-series/P1.25 216 inch.png",
    resolution: { width: 3840, height: 2160 },
    pixelPitch: 1.25,
    pixelDensity: 640000,
    cabinetDimensions: { width: 4800, height: 2700 }, // 16:9 aspect ratio, ~216" diagonal
    moduleDimensions: { width: 320, height: 160 },
    moduleResolution: { width: 256, height: 128 },
    moduleQuantity: 4,
    brightness: 600,
    contrastRatio: "10000:1",
    refreshRate: 3840,
    environment: "Indoor",
    maxPowerConsumption: 3000,
    avgPowerConsumption: 1600,
    weightPerCabinet: 240,
    ledType: "COB LED",
    viewingAngle: "H 160°, V 160°",
    ipGrade: "IP31",
    operatingTemperature: "-20°C~60°C",
    operatingHumidity: "10-85%RH",
    storageTempHumidity: "-20°C~60°C, 10-85%RH",
    driveMethod: "1/48",
    serviceAccess: "Front service",
    cabinetMaterial: "Die-Cast Aluminum",
    colorTemperature: "5000-12000 Optional",
    videoInterface: "HDMIX3(2 Input, 1 output) USB3.0 USB2.0x2 RF",
    signalInterface: "RJ45, USB-B, Wifi, Bluetooth, Infrared",
    audioInterface: "Internal 2x5w speakers, External Audio Interface",
    installationMode: "Moveable(standard-stand)/ Wall mount (optional)",
    controller: "Novastar",
    lifeTime: 100000,
    price: 4800000,
    siChannelPrice: 4560000,
    resellerPrice: 4320000,
    prices: {
      cabinet: { endCustomer: 4800000, siChannel: 4560000, reseller: 4320000 }
    }
  },
];

export const categories = Array.from(new Set(products.filter(p => p.enabled !== false).map(p => p.category)));
